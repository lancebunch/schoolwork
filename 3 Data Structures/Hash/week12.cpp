...
{
public:
   FHash(int numBuckets, float min, float max) throw(const char *) : Hash<float>(numBuckets), min(min), max(max) {}
   FHash(const FHash &rhs) throw(const char *) : Hash<float>(rhs), min(rhs.min), max(rhs.max) {}
