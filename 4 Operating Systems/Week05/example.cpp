...
/***********************************************************
 * MatrixCalculationUnit
 * A data-structure to contain the the data necessary
 * for calculating one value of the resulting matrix
 **********************************************************/
struct MatrixCalculationUnit
{
   int desRow;
   int desCol;
};

void *matrixCalculation(void *data);
...
bool multiply(MatrixMultiplication &m)
{
   ...
   for (int i = 0; i < numRowDes; i++)
   {
      ...
      {
         int pos = i * numColDes + j;
         data[pos].desRow = i;
         data[pos].desCol = j;
         pthread_create(&(threads[pos]), NULL, matrixCalculation, (void *)(data + pos));
      }
   }
...
/***********************************************************************************
 * matrixCalculation
 * Function to calculate one value of the resulting matrix
***********************************************************************************/
void *matrixCalculation(void *data)
{
   MatrixCalculationUnit *unit = (MatrixCalculationUnit *)data;
   int dest = unit->desRow * matrix.numColRHS + unit->desCol;
   int sourceRow = unit->desRow * matrix.numColLHS;
   for (int i = 0; i < matrix.numColLHS; i++)
   {
      matrix.arrayDestination[dest] += matrix.arrayLHS[sourceRow + i] * matrix.arrayRHS[i * matrix.numColRHS + unit->desCol];
   }
   return 0;
}
